...
public:
	...
	: QWidget(NULL, "dcguiqtdock",  WType_TopLevel | WStyle_Customize | WStyle_NoBorder | WStyle_StaysOnTop | WMouseNoMask), iconObject(object)
//	: QLabel( 0, "psidock", WMouseNoMask ), iconObject(object)
	{
		...
		classhint.res_name  = (char*)"dcguiqtdock";
		...
		XFree( hints );
		setBackgroundMode(X11ParentRelative); // hopefully fix the mask problem
	...
	{
		// Taken straight from KSystemTray..
		// Thankyou KDE!
		if ( !qApp->focusWidget() ) {
			XEvent ev;
			memset(&ev, 0, sizeof(ev));
